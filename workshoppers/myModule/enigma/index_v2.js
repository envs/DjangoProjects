...
class Enigma{
    ...
    encode (str) {
        // Unique generation of Initialization Vector (IV)
        // crypto.pbkdf2Sync(password, salt, iterations, keylen, digest);
        const iv = crypto.pbkdf2Sync(this.key, crypto.randomBytes(16),
                                    10000, 16, 'sha512');
        const key = Buffer.from(this.key, 'binary');
        const cipher = crypto.createCipheriv("aes-256-ctr", key, iv);
        let encodedText = cipher.update(str, 'utf8', 'base64');
        encodedText += cipher.final();
        return encodedText + "." + iv.toString('base64');     //-> For demonstration purpose
        //return encodedText
    }
    decode (str) {
